...
const ChatGPT = () => {
    ...
    const senMessage = (message) => {
        ...
        const headers = {
            ...
            'Authorization': `Bearer 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`
        };
                                            ...
                                            <div>
                                                {chatLog.map((message, index) => (
                                                    <div key={index} className={`d-flex ${message.type === 'user' ? 'justify-content-end' : 'justify-content-start '}`}>
                                                        <div className={`${message.type === 'user' ? 'bg-user' : 'bg-bot'} `}>
                                                            {message.message}
                                                        </div>
                                                    </div>
                                                ))}
                                                {isLoading && (
                                                    <div key={chatLog.length} className="d-flex justify-content-start">
                                                        <div>
